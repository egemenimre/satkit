...
from org.orekit.utils import Constants, TimeStampedPVCoordinates, IERSConventions
...
def kep_propagator():
    ...
    initialOrbit = KeplerianOrbit(pos_vel_time,
                                  inertialFrame,
...
def elev_event_inputs_outputs():
    ...
    # Nominal case
    nominal_search_interval = TimeInterval(AbsoluteDateExt("2014-01-01T23:30:00.000", utc),
                                           AbsoluteDateExt("2014-01-02T23:30:00.000", utc))
    nominal_output = [
        TimeInterval(AbsoluteDateExt("2014-01-01T23:31:49.55286151707821Z", utc),
                     AbsoluteDateExt("2014-01-01T23:42:49.64917646999244Z", utc)),
        TimeInterval(AbsoluteDateExt("2014-01-02T01:11:23.26932478260755Z", utc),
                     AbsoluteDateExt("2014-01-02T01:18:20.66899834602248Z", utc)),
        TimeInterval(AbsoluteDateExt("2014-01-02T11:38:01.53899331924902Z", utc),
                     AbsoluteDateExt("2014-01-02T11:48:04.87831264440055Z", utc)),
        TimeInterval(AbsoluteDateExt("2014-01-02T13:15:29.16139702783088Z", utc),
                     AbsoluteDateExt("2014-01-02T13:25:06.87081687644551Z", utc)),
        TimeInterval(AbsoluteDateExt("2014-01-02T22:35:21.16795871369053Z", utc),
    ...
    begin_with_pass_search_interval = TimeInterval(AbsoluteDateExt("2014-01-01T23:35:00.000", utc),
                                                   AbsoluteDateExt("2014-01-02T23:30:00.000", utc))
    begin_with_pass_output = deep_copy_intervals(nominal_output)
    begin_with_pass_output[0] = TimeInterval(AbsoluteDateExt("2014-01-01T23:35:00.000", utc),
                                             AbsoluteDateExt("2014-01-01T23:42:49.64917646999244Z", utc))

    input_output_tuples.append((begin_with_pass_search_interval, begin_with_pass_output))

    # Search ends inside a pass
    end_with_pass_search_interval = TimeInterval(AbsoluteDateExt("2014-01-01T23:30:00.000", utc),
                                                 AbsoluteDateExt("2014-01-02T22:38:00.000", utc))
    end_with_pass_output = deep_copy_intervals(nominal_output)
    end_with_pass_output[-1] = TimeInterval(AbsoluteDateExt("2014-01-02T22:35:21.16795871369053Z", utc),
                                            AbsoluteDateExt("2014-01-02T22:38:00.000", utc))
    ...
    short_search_interval_1 = TimeInterval(AbsoluteDateExt("2014-01-01T23:35:00.000", utc),
                                           AbsoluteDateExt("2014-01-01T23:40:00.000", utc))
    short_output_1 = [TimeInterval(AbsoluteDateExt("2014-01-01T23:35:00.000", utc),
                                   AbsoluteDateExt("2014-01-01T23:40:00.000", utc))]
    input_output_tuples.append((short_search_interval_1, short_output_1))

    # Search interval is outside passes - no pass to be found
    no_pass_search_interval = TimeInterval(AbsoluteDateExt("2014-01-02T20:35:00.000", utc),
...
def test_elevation_events():
    ...
    for search_interval, expected_intervals in elev_event_inputs_outputs():
        # find passes
        passes = gnd_pass_finder(search_interval, kep_propagator(), gnd_location(), elevation, earth())
